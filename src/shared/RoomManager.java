...
public class RoomManager {
	...
	private static String stripControlCodes(String original) {
		String allowedChars = "0123456789";
		for (int i = 0; i < original.length() - 1; i++) {
			String curchar;
			if (i + 1 < original.length())
				curchar = original.substring(i, i + 1);
			else
				curchar = original.substring(i);
			...
			if (curchar.equals("\u0003")) {
				...
				for (int j = i + 1; j < original.length() - 1; j++) {
					if (j + 1 < original.length())
						curchar = original.substring(j, j + 1);
					else
						curchar = original.substring(j);

					if (numStr.length() > 0 && curchar.equals(","))
						commaPos = j;
		...
		if (!main.getDisplay().isDisposed()) {
			main.getDisplay().asyncExec(new Runnable() {
				public void run(){
					...
					Customs c = new Customs(); // Create a Customs object, used for
												// color-checking
					String allowedChars = "0123456789"; // Create a string, used to check
														// for valid colors after
					...
					for (int i = 0; i < m.getContent().length() - 1; i++) {
						String curchar;
						if (i + 1 < m.getContent().length())
							curchar = m.getContent().substring(i, i + 1);
						else
							curchar = m.getContent().substring(i);
						...
						if (curchar.equals("\u0003")) { // Color control code
							...
							for (int j = i + 1; j < m.getContent().length() - 1; j++) {
								if (j + 1 < m.getContent().length())
									curchar = m.getContent().substring(j, j + 1);
								else
									curchar = m.getContent().substring(j);
							...
							if (numStr.length() > 0)
								fgColNum = Integer.parseInt(numStr);
							else
								continue;
			
							if (bgNumStr.length() > 0)
								bgColNum = Integer.parseInt(bgNumStr);
							...
							if (commaPos != -1)
								totalNumStrLens += bgNumStr.length() + 1;
			
							StyleRange styleRange = new StyleRange();
							styleRange.start = r.getOutput().getCharCount()
									- strippedLine.length() + i - totalDispos;
							styleRange.length = nextColCode - i - totalNumStrLens - 1;
							if (styleRange.length < 0)
								styleRange.length = strippedLine.length() - i + totalDispos;
						...
						if (curchar.equals("\u0002")) { // Bold control code
							if (!applied[0]) {
								...
								int nextNormalCode = m.getContent()
										.indexOf("\u000F", i + 1);
			
								int nextCtrlCode = Math.min(nextBoldCode, nextNormalCode);
			
								if (nextBoldCode == -1)
									nextCtrlCode = nextNormalCode;
								if (nextNormalCode == -1)
									nextCtrlCode = nextBoldCode;
								...
								if (styleRange.length < 0)
									styleRange.length = strippedLine.length() - i
											+ totalDispos;
